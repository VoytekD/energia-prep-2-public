...
# Kluczowe tabele wejściowe (wg dotychczasowego health)
INPUT_TABLES = [
    ("input", "date_dim"),
    ("input", "konsumpcja"),
    ("input", "produkcja"),
...
# Kandydaci na tabelę cen (legacy + docelowa)
PRICES_CANDIDATES = [
...
def _connect() -> psycopg.Connection:
    ...
    # wymuś UTC na sesji (prezentacja timestamptz)
...
def _group_schema(conn: psycopg.Connection) -> Dict[str, Any]:
    ...
    status = "up" if not (missing_schemas or missing_tables) else "degraded"
    return {
        "status": status,
        "missing": {
