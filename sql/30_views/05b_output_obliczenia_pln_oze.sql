-- =====================================================================================================
--  Scenariusz B: OZE (bez magazynu)
--  IDENTYCZNY schemat jak 05a/05c. JEDYNA różnica: źródło poboru (CTE base).
--  Poniżej domyślnie używam e_deficit_mwh z output.energy_base, ale:
--  >>> TU PODMIEŃ ŹRÓDŁO, jeśli wolisz inne (np. e_delta_mwh, itp.) <<<
-- =====================================================================================================

CREATE SCHEMA IF NOT EXISTS output;
DROP VIEW IF EXISTS output.obliczenia_pln_oze;

CREATE VIEW output.obliczenia_pln_oze AS
WITH
dd AS (
...
base AS (
  SELECT
    ...
    -- Scenariusz B (domyślnie): pobór = deficyt po bilansie własnej generacji
    -- >>> TU PODMIEŃ ŹRÓDŁO, jeśli chcesz inny wskaźnik poboru <<<
    ROUND(COALESCE(eb.e_deficit_mwh,0), 6)::numeric AS pobor_mwh
  FROM dd d
  LEFT JOIN output.energy_base eb ON eb.ts_utc = d.ts_utc
),
-- ▲▲▲ KONIEC: ŹRÓDŁO POBORU ▲▲▲

-- LCOE OZE (identycznie jak 05a)
base_oze AS (
  SELECT
    b.ts_utc,
...
-- Dalej: identycznie jak 05a (ceny/dystr/system/fisk) — bez zmian
ceny_dyn AS (
...
-- złożenie + j2 identycznie jak 05a
joined AS (
  SELECT
    b.*,
