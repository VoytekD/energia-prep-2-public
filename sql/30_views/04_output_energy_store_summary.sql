...
--  • Grid import/export: z 01 (po magazynie), plus diagnostyka curtailed/unserved.
--
...
br AS (
  SELECT
    ...
    -- pojemności (jeśli chcesz brać z 01; tu użyjemy params, ale zostawiam dla analityki)
...
-- 03: SOC ARBI + ładowanie (01+spill) i extra rozładowanie + idle
ar AS (
  SELECT
    y.ts,
    COALESCE(y.soc_arbi_mwh,0)::numeric                AS soc_arbi_mwh,
    COALESCE(y.e_arbi_ch_in_mwh,0)::numeric            AS arbi_ch_net_total_mwh,    -- NET z 01+spill (po ograniczeniach 03)
    ...
    COALESCE(y.retention_per_step,1)::numeric          AS r_step_arbi               -- z 03
...
-- Rozbicie źródeł ładowania ARBI: ile z 01, ile ze spill (02)
split_arbi_ch AS (
  SELECT
    j.*,
...
flows AS (
  SELECT
    ...
    -- Wejścia GROSS po stronie AC (diagnostyka) — liczymy z pól źródłowych X.*, NIE z aliasów powyżej
...
SELECT
  ...
  -- GRID po magazynie (z 01)
  ROUND(f.grid_import_after_bess_mwh, 6)   AS grid_import_mwh,
