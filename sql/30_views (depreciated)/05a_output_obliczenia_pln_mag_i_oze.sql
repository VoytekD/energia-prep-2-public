-- =====================================================================================================
--  Scenariusz A: Magazyn + OZE
--  Jedyny punkt różnicy względem 05b/05c: źródło poboru (pobor_mwh) w CTE base.
--  Tutaj: import z sieci po magazynie z output.energy_store_summary.grid_import
-- =====================================================================================================

CREATE SCHEMA IF NOT EXISTS output;
DROP VIEW IF EXISTS output.obliczenia_pln_mag_i_oze;

CREATE VIEW output.obliczenia_pln_mag_i_oze AS
WITH
-- Oś czasu
dd AS (
...
base AS (
  SELECT
    d.ts_utc, d.ts_local, d.year, d.month, d.day, d.dow, d.hour, d.is_workday,
    -- Scenariusz A: pobór to import z sieci po magazynie
    ROUND(COALESCE(ess.grid_import_mwh,0), 6)::numeric AS pobor_mwh
  FROM dd d
  LEFT JOIN output.energy_store_summary ess ON ess.ts = d.ts_utc
),
-- ▲▲▲ KONIEC: ŹRÓDŁO POBORU ▲▲▲

-- LCOE OZE (generacja wg 00 + stawki z params.form_lcoe)
base_oze AS (
  SELECT
    b.ts_utc,
...
-- (B) dystrybucja zmienna (strefy + jakościowa)
month_map_dyst AS (
